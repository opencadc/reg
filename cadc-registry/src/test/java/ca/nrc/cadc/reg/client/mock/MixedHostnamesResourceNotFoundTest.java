 ...
 * 
 */
public class MixedHostnamesResourceNotFoundTest
    {
    private static final Logger log = Logger.getLogger(MixedHostnamesResourceNotFoundTest.class);
    static {
    ...
    public void testRegistryResourceNotFoundOnce()
        ...
        //
        // The registry client should have called the 'resource-caps' endpoint on the bad registry twice, once to try to populate the cache, and a second time to try to get the content without using the cache.
        mockServer.verify(
            request()
                .withPath(
                    "/bad-registry-one/resource-caps"
    ...
    public void testRegistryResourceNotFoundTwice()
        ...
        //
        // The registry client should call the 'resource-caps' endpoint on the bad registry twice, once to try to populate the cache, and a second time to try to get the content without using the cache.
        mockServer.verify(
            request()
                .withPath(
                    "/bad-registry-one/resource-caps"
