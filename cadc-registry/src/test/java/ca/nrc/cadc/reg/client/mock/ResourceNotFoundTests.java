...
extends MockServerTestBase
    {
    private static final Logger log = Logger.getLogger(ResourceNotFoundTests.class);
    static {
    ...
    @Test
    public void testLocalhostMultipleNotFound()
        throws Exception {
        ...
        RegistryClient registryClient = buildRegistryClient(
            "http://localhost:1080/not-found-001",
            "http://localhost:1080/not-found-002",
            "http://localhost:1080/not-found-003"
            );
        ...
        //
        // The registry client should have called the 'resource-caps' endpoint on all of the registries twice, once to try to populate the cache,
        // and a second time to try to get the content without using the cache.
        mockServer.verify(
            request()
                .withPath(
    ...
    @Test
    public void testLocalhostMixedNotFound()
        throws Exception {
        ...
        // BUT the cache is indexed by the hostname only, not the full URL, so http://localhost:1080/not-found-001 and http://localhost:1080/good-registry-002 share the same cache file.
        // This means that the client finds the cached response for 'localhost/good-registry-002' and doesn't need to call the bad registry.
        mockServer.verify(
            request()
                .withPath(
                    "/not-found-001/resource-caps"
    ...
    @Test
    public void testMixedHostMultipleNotFound()
        throws Exception {

        //
        // Three bad registries on different hostnames.
        RegistryClient registryClient = buildRegistryClient(
            "http://testhost-001:1080/not-found-001",
            "http://testhost-002:1080/not-found-002",
            "http://testhost-003:1080/not-found-003"
            );
        ...
        //
        // The registry client should have called the 'resource-caps' endpoint on all three of the registries twice, once to try to populate the cache,
        // and a second time to try to get the content without using the cache.
        mockServer.verify(
            request()
                .withPath(
    ...
    @Test
    public void testMixedHostMixedNotFound()
        throws Exception {
        ...
        //
        // The registry client should have called the bad registry endpoint twice, once to try to populate the cache,
        // and a second time to try to get the content without using the cache.
        mockServer.verify(
            request()
                .withPath(
        ...
        //
        // The registry client should have called the second registry endpoint once to get the result and populate the cache.
        mockServer.verify(
            request()
                .withPath(
                    "/good-registry-002/resource-caps"
        ...
        //
        // The registry client should have found a cached value for the second registry, and so it should not have needed to call the second registry.
        mockServer.verify(
            request()
                .withPath(
                    "/good-registry-002/resource-caps"
