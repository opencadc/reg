...
/**
 * A set of Junit tests that use a MockServer to test a how the RegistryClient handles an unknown service ivoid. 
 * See https://www.mock-server.com/
 * 
 */
public class MixedRegistriesUnknownServiceTest
    {
    private static final Logger log = Logger.getLogger(MixedRegistriesUnknownServiceTest.class);
    static {
    ...
    public void setupMockServer() {
        ...
        //
        // Setup the bad 'resource-caps' response.
        mockServer.when(
            request()
                .withPath(
        ...
        //
        // Setup the 'unknown-capabilities' response.
        mockServer.when(
            request()
                .withPath(
    ...
    @Test
    public void testUnknownServiceOnce()
        throws Exception {
        
        //
        // Try to get the good service capabilities.
        try {
        ...
        // The registry client will keep the results, even though the good-service isn't in the list.
        mockServer.verify(
            request()
                .withPath(
                    "/bad-registry-one/resource-caps"
        ...
        //
        // The registry client should not call the other registries.
        mockServer.verify(
            request()
                .withPath(
                    "/good-registry-one/resource-caps"
        ...
        //
        // The registry client should not have found the 'unknown-capabilities' endpoint.
        mockServer.verify(
            request()
                .withPath(
                    "/unknown-service/unknown-capabilities"
        ...
        //
        // The registry client should not have found the 'good-capabilities' endpoint.
        mockServer.verify(
            request()
                .withPath(
                    "/good-service/good-capabilities"
    ...
    @Test
    public void testUnknownServiceTwice()
        throws Exception {
        ...
        mockServer.verify(
            request()
                .withPath("/good-registry-one/resource-caps"),
                VerificationTimes.exactly(0)
            );        
        mockServer.verify(
            request()
                .withPath("/good-registry-two/resource-caps"),
                VerificationTimes.exactly(0)
            );        
